...
- The Ninebot `trail` points observed for this account match online GPX viewers
  such as gpx.studio without GCJ-to-WGS correction, so the exporter treats raw
  `trail` coordinates as `*_wgs84.*`.
- `*_gcj02.*` files are generated by applying a WGS-84 to GCJ-02 transform and
  are intended for Chinese-map apps that expect GCJ-02 coordinates.
